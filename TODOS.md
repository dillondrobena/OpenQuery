...
Deferred work with context, captured during pre-implementation engineering review (2026-07-09) and subsequent design/security reviews.
...
## Keyboard navigation for the viewer graph (design review, 2026-07-10)
...
- **Context:** viewer/app.js registers onNodeClick/onLinkClick handlers; a keyboard layer can reuse renderReceipt() directly (the same path automated QA uses).
...
- **Context:** Deferred in the original product design because trust perception drove the Skill+CLI form factor. The alias-only architecture means the original objection is technically defused; re-decide with adoption data.
